...
function SheetContent(props: SheetContentProps) {
  ...
  const sideClasses = {
    ...
    top: 'inset-x-0 top-0 h-1/3 max-h-full',
    bottom: 'inset-x-0 bottom-0 h-1/3 max-h-full',
  };
  return (
    <SheetPortal>
      <SheetOverlay />
      <SheetPrimitive.Content
        ...
        className={cn(
          ...
          sideClasses[sheetSide],
          className,
          // 让全屏菜单时内容相对定位，方便绝对定位关闭按钮
          sheetSide === 'left' && className?.includes('inset-0') ? 'relative' : '',
        )}
        {...rest}
      >
        {children}
        {/* 统一关闭按钮样式和菜单按钮 */}
        <SheetPrimitive.Close className="absolute top-4 right-4 z-50 p-2 rounded bg-white border shadow size-10 flex items-center justify-center opacity-80 transition-opacity hover:opacity-100 focus:ring-2 focus:ring-offset-2">
